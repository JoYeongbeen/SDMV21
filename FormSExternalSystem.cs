...
namespace SDM
{
  public partial class FormSExternalSystem : Form
  {
    ...
    public SExternalSystem SExternalSystem;
    private SBR SBR;
    private SDA SDA;

    public FormSExternalSystem()
    {
      InitializeComponent();
    }

    private void FormSExternalSystem_Load(object sender, EventArgs e)
    {
      this.SBR = new SBR();
      this.SDA = new SDA();
      this.SetControl();
    ...
    private void SetControl()
    {
      ...
      this.BtnSave.Visible = SCommon.HasPermission(this.SExternalSystem);
    }

    private void SelectComponent()
    {
      this.TbxID.Text = this.SExternalSystem.ProgramID;
      this.TbxName.Text = this.SExternalSystem.Name;      
      this.TbxDescription.Text = this.SExternalSystem.Description;
      this.TbxSpecFile.Text = this.SExternalSystem.SpecFile;
      this.TbxRegistered.Text = SCommon.GetRegistered(this.SExternalSystem);
      this.TbxLastModified.Text = SCommon.GetLastModified(this.SExternalSystem);

      //Callee List
      List<SCallee> calleeList = this.SDA.SelectCalleeList(this.SExternalSystem.GUID);
      ...
      this.SExternalSystem.CalleeList = calleeList;
    ...
    private void LbxMessageList_DragDrop(object sender, DragEventArgs e)
    {
      ...
      SCallee sCallee = new SCallee(SComponentType.ExternalSystem, this.SExternalSystem.GUID, componentType, calleeComponent.GUID, this.SBR.GetComponentFullPath(calleeComponent), string.Empty);
      this.SExternalSystem.CalleeList.Add(sCallee);
    ...
    private void BindCalleeListBox()
    {
      ...
      this.LbxMessageList.DataSource = this.SExternalSystem.CalleeList;
    ...
    private void BtnMoveUpCall_Click(object sender, EventArgs e)
    {
      if (this.LbxMessageList.SelectedItem != null)
      {
        ...
        if (currentIndex > 0)
        {
          this.SExternalSystem.CalleeList.Insert(currentIndex - 1, selectedCallee);
          this.SExternalSystem.CalleeList.RemoveAt(currentIndex + 1);
    ...
    private void BtnMoveDownCall_Click(object sender, EventArgs e)
    {
      if (this.LbxMessageList.SelectedItem != null)
      {
        ...
        {
          this.SExternalSystem.CalleeList.Insert(currentIndex + 2, selectedCallee);
          this.SExternalSystem.CalleeList.RemoveAt(currentIndex);
    ...
    {
      this.SExternalSystem.CalleeList[selectedIndex].Comment = comment;
      this.BindCalleeListBox();
    }

    private void BtnAddComment_Click(object sender, EventArgs e)
    {
      SCallee sCallee = new SCallee(SComponentType.ExternalSystem, this.SExternalSystem.GUID, "New Comment");
      this.SExternalSystem.CalleeList.Add(sCallee);
    ...
    private void BtnDeleteCall_Click(object sender, EventArgs e)
    {
      if (this.LbxMessageList.SelectedItem != null)
      {
        this.SExternalSystem.CalleeList.Remove(this.LbxMessageList.SelectedItem as SCallee);
        this.BindCalleeListBox();
      }
    }
    ...
    private void UpdateComponent()
    {
      ...
      this.SExternalSystem.ProgramID = this.TbxID.Text.Trim();
      this.SExternalSystem.Name = this.TbxName.Text.Trim();
      this.SExternalSystem.Description = this.TbxDescription.Text;
      this.SExternalSystem.SpecFile = this.TbxSpecFile.Text;

      SCommon.SetDateDesigner(this.SExternalSystem, false);
      this.TbxRegistered.Text = SCommon.GetRegistered(this.SExternalSystem);
      this.TbxLastModified.Text = SCommon.GetLastModified(this.SExternalSystem);

      //Callee List
      this.SDA.DeleteCalleeList(this.SExternalSystem.GUID);

      foreach (SCallee callee in this.SExternalSystem.CalleeList)
        this.SDA.InsertCallee(callee);

      this.SDA.UpdateExternalSystem(this.SExternalSystem);
      this.ComponentChanged(this.SExternalSystem);
    ...
    {
      this.MoveComponent(this.SExternalSystem.GUID);
    }
    ...
    private void LblLastModified_DoubleClick(object sender, EventArgs e)
    {
      ...
      {
        SCommon.SetLastModifiedTemp(this.SExternalSystem);
        this.TbxLastModified.Text = SCommon.GetLastModified(this.SExternalSystem);
        this.SDA.UpdateExternalSystemLastModifiedTemp(this.SExternalSystem);
